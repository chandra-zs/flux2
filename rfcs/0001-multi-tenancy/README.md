...
to a cluster. These controllers are subject to authorisation on two counts:
 ...
 - when accessing the Kubernetes resources that specify a particular "apply" operation -- for
   example, the `GitRepository` object that gives access to the piece of configuration to be
   applied; and,
